...
def create_no_goals_df(df, df_goals, uniek_id_list, column_selection):
    ...
    for uniekid in uniek_id_list:
        ...
        df_no_goals = df_no_goals.loc[~df_no_goals['minute'].isin(goals_minutes)]
        df_no_goals['goal'] = 0
...
def get_events_per_id(df, events_selection_list, uniek_id_list):
    ...
    for uniekid in uniek_id_list:
        ...
        events_per_match_select = events_per_match_select.loc[events_per_match_select['minute'] >= 1]   # remove starting positions 
        events_per_match_select = events_per_match_select[events_selection_list]
...
def events_feature_engineering(df):
    ...
    df['in_de_16'] = 0
    df.loc[(df['location_x'] >= 102.62) & (df['location_y'] >= 18) & (df['location_y'] <= 62), 'in_de_16'] = 1
    df.loc[df['under_pressure'] == True, 'under_pressure'] = 1
...
def aggregate_events(df, groupby_columns , agg_type_max):
    ...
    for column in x_vars:
        if agg_type_max:
            col_sum = df.groupby(groupby_columns)[column].max()
        else:
            col_sum = df.groupby(groupby_columns)[column].sum()   
